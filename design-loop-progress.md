...
User reported the deployed site (https://next-js-pokemon-alpha.vercel.app, real card data via their API key) feels cramped top-to-bottom — something none of the 4 rounds' critics could catch, since they only ever saw "0 cards" locally. Diagnosed via `getBoundingClientRect()` on the live deployment: every `border-t` section divider (chart section, Price alerts, "How the price checker works," "Available card IDs", footer bottom row) had exactly **1px** between the divider and the heading below it — round 3's cleanup removed the old `pt-8` but never replaced it with the small rhythm token. Fixed by adding `pt-3` (12px) after all 6 dividers. Shipped (`0868fd1`).

### Round 6 (post-ship, live user feedback — the general rule was still wrong)
User pushed round 5 and reported spacing *still* reads cramped in several sections (product hero info stack, Price alerts, All cards). Real cause, this time: the whole `bar.md`/`design-system.md` spacing model only ever had two rhythm tokens (80px section, 12px element) because 099 SUPPLY's own reference never stacks more than 2 things — but real CardTrace pages stack 4-6 distinct blocks (title, meta, price, description, links, button) through that same 12px tier. Researched standard practice (8-point grid — Material Design, IBM Carbon; see design-system.md's Spacing section for sources) and added a third tier: **Group (24px)**, for 3+ distinct blocks stacked together or a heading followed by a large self-contained widget. Tight (12px) is now reserved for genuinely paired items only (a heading + its own one-line subtitle).

Bumped to Group (`space-y-3`→`space-y-6` / 12px→24px) in: `alert-subscribe.tsx`, `site-footer.tsx` (both stacks), `about/page.tsx`, `page.tsx` (hero section, "All cards" section), `tools/price-checker/page.tsx` (card-result block), `products/[slug]/page.tsx` (hero info stack, price-alerts section). Left at Tight (12px) where genuinely just 2 coupled items: `page.tsx`'s "How the price checker works" (heading + one paragraph), and all grid gutters between repeated homogeneous items (card tiles, ID list). `tsc`/`build` pass. Not yet committed/pushed — pending.
