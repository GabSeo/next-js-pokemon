...
type ActiveTier = { listings: EbayActiveListing[]; median: number };
 ...
 * page — it's caught locally and degrades that one tier to a search-link
 * fallback, the same resilience shape lib/cards.ts uses for apitcg.
 */
async function fetchActiveTier(card: Card, condition: EbayCondition): Promise<ActiveTier | null> {
  try {
    const listings = await searchActiveListings(card, condition);
    if (listings.length === 0) return null;
    const med = median(listings.map((l) => l.price));
    return med === null ? null : { listings, median: med };
  } catch (err) {
...
/**
 * One row per condition tier, active (real, eBay Browse API) and sold
 * (illustrative — see lib/illustrative.ts, no API exposes sold eBay data to
 * this developer-program tier) regrouped on the same line so they're
 * directly comparable. The ROI estimate below the table is computed from
 * active-listing medians only — sold data, being fabricated, never feeds a
 * number presented as computed/real.
 */
export async function GradedMarketPanel({ card }: { card: Card }) {
  ...
  const roi = psa10Active && rawActive ? gradingRoi(psa10Active.median, rawActive.median) : null;
    ...
    <div className="rounded-lg border-2 border-black bg-card-surface p-6 shadow-hard-md">
        ...
        <table className="w-full min-w-[520px] border-collapse text-sm">
          <thead>
            <tr className="border-b-2 border-black text-left text-xs font-black tracking-[0.3px] text-muted-text uppercase">
              ...
              <th className="py-2 pr-3">Active — real, last 3</th>
              <th className="py-2">
                  ...
                  <td className="py-3 pr-3 align-top">
                    {active ? (
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-xs font-bold text-muted-text underline underline-offset-2 hover:text-foreground"
                      >
                        Shop {condition} listings ↗
                      </a>
                    )}
                  </td>
      ...
      <div className="mt-5 rounded-md border-2 border-black bg-muted-surface p-4">
        <div className="mb-1 flex flex-wrap items-center gap-2 text-xs font-black tracking-[0.3px] text-muted-text uppercase">
          ...
          <IllustrativeTag label="Estimate, active listings only" />
        </div>
        {roi !== null ? (
          <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
            ...
            <span className="text-xs font-bold text-muted-text">
              {card.currency} {rawActive!.median.toLocaleString()} raw + {card.currency} {DEFAULT_PSA_GRADING_COST_USD} grading
              vs {card.currency} {psa10Active!.median.toLocaleString()} PSA 10, today&apos;s active listings.
            </span>
          </div>
        ) : (
          <p className="text-xs font-bold text-muted-text">
            Needs real PSA 10 and Raw active-listing data to compute — unavailable right now.
          </p>
        )}
      </div>
    </div>
